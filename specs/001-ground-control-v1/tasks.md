...
- [ ] T038 [P] [US1] Record Jira fixtures in `fixtures/jira/` — search pages, changelogs, status categories across `new`/`indeterminate`/`done`
- [ ] T039 [P] [US1] Record GitHub fixtures in `fixtures/github/` — PRs with review threads, check runs, branch comparisons
- [ ] T040 [P] [US1] Record git fixtures in `fixtures/git/` — porcelain v2 output, worktree lists, CRLF variants, paths with spaces and non-ASCII characters
