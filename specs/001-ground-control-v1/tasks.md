...
- [x] T039 [P] [US1] Record GitHub fixtures in `fixtures/github/` — recorded and replayed the same way. Re-recorded 2026-08-16 from an active **public** repository (50 pull requests, 92 branches), using a fine-grained token scoped to public repositories read-only, stored under its own connection so the operator's own credential was never touched. That closes the gap this task carried: `reviewDecision` (`REVIEW_REQUIRED`, `APPROVED`), review threads (7 resolved, 1 unresolved, 7 outdated) and check runs (`SUCCESS`, `FAILURE`, `SKIPPED`, `NEUTRAL`, and 9 still running) are now covered by real payloads. **It found a bug on its first run**: the replay test asserted kebab-case `review-required`, which nothing in the system has ever produced — and had never run, because the previous recording had no reviewed PRs to enter the loop with. **Still uncovered by real data**: `CHANGES_REQUESTED`, which no PR in the recording carried
