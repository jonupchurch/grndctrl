...
- [ ] T040 [P] [US1] Record git fixtures in `fixtures/git/` — **not done, and partly unnecessary.** The recorder is HTTP-based; local git needs a second one that captures command output. The specific cases this task names — CRLF variants, paths with spaces, non-ASCII — are already covered by `packages/core/test/providers/git-windows.test.ts` (T056), which found three real defects. What a recording would add is porcelain shapes nobody thought to hand-write, which is real but smaller value than T038/T039 carried
