...
- [x] **The one thing that must *not* be removed alongside is called out** — the first-run egress entries for the native module download, and the per-lane error boundaries
...
- [x] A real decision point is marked in the task list rather than buried (T014 — look at the board before dismantling the engine behind it)
...
- [ ] **Version number.** Recommendation **0.4.0**. This breaks a published MCP tool surface; 1.0.0 is available if a harder signal is wanted, but claims a stability this is not the moment to claim.
...
- [ ] **The correlation badge column** (T006). With only the agent correlation left it is a one-badge column. Decide by looking at the board at T014.
- [ ] **The two-column board layout** (T014). One work lane in the main column may or may not still want a side column.
